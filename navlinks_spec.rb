  ...
  context "private methods" do
    describe "#send(:generate_html)" do
      it "returns a properly nested unordered list" do
        ...
        navlinks = Navlinks.new
        html = navlinks.send(:generate_html, "other", ['/', '/aboutjwu/', '/aboutjwu/academicdirectories/'])
        expect(html).to eq(proper_html)
      end
    end

    describe '#send(:find_records)' do
      context 'given a campus and path' do
        it 'returns the correct list of records' do
          navlinks = Navlinks.new
          found_records = navlinks.send(:find_records,'other', '/aboutjwu/academicdirectories/')
    ...
    describe "#send(:breakdown_path)" do
      it "returns [/] if path == /" do
        path = "/"
        navlinks = Navlinks.new
        expect(navlinks.send(:breakdown_path,path)).to eq([path])
      end
      ...
      it "returns all path parts of a given path" do
        ...
        navlinks = Navlinks.new
        expect(navlinks.send(:breakdown_path,path).size).to eq(3)
        expect(navlinks.send(:breakdown_path,path)).to eq(['/', '/aboutjwu/', path])
      end
    end
  end
end
